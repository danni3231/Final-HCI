...
var banderas = [Colombia, Argentina, Bolivia, Peru, Guatemala, Venezuela, Ecuador, Mexico];
shuffle(banderas);
...
function createCanvass(pais){
  ...
  image.setAttribute('src', './data/images/level1/BanderasPaises/'+pais.silueta);
...
function createFlags(pais) {
  ...
  image.setAttribute('src', './data/images/level1/BanderasPaises/'+pais.bandera);
  ...
  if (bandera == territorio) {
    for (let i = 0; i < managerDrag.drops.length; i++) {
      if (managerDrag.drops[i].src.includes(bandera.nombre)) {
        managerDrag.drops[i].src="./data/images/level1/BanderasPaises/"+dragElem.siluetaPintada;
